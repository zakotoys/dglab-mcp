...
- Package as scoped `@zakotoys/dglab-mcp` with the version declared in `package.json`, a `dglab-mcp` executable, lockfile, shebang-preserving ESM build, `prepublishOnly` verification, and `GPL-3.0-only` licensing.
